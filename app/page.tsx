...
export default function Curriculum(){
  ...
  useEffect(()=>{
    const item =localStorage.getItem("content-cv")
    if (!!item){
  ...
  useEffect(()=>{
    ...
    localStorage.setItem("content-cv",item);
  },[profile])
    ...
    <Layaut>
      <div>
        ...
        <Form.Container id="languages" label='Languages'>
          ...
          <Form initialState={{}} className="skills" onSubmit={(data)=>handleOnSubmitLanguage(data)}>
            <Form.TextField name="name" label="skill" required  />
            <Form.RangeField min={1} max={5} name="level" required  />
            <Form.Submit name="submit" label="save" />
          </Form>
